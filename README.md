...
* undefined
	* 정의되지 않은 변수
	* __변수를 선언했지만 **초기화를 하지 않은** 경우__
* null
	* __값은 저장했지만 존재하지 않을 때__
 ...
 * 배열을 객체 형식으로 선언할 수도 있다
 ...
 var arr2 = new Array(10); //파라미터 1개일 때는 __배열의 크기__를 지정
 var arr3 = new Array(1,2,3); //파라미터 n개일 때는 크기 n짜리 배열에 __초기화__까지 한 것
 ```
 * 배열의 함수
 ```javascript
 //push() : 배열의 마지막에 값을 추가
